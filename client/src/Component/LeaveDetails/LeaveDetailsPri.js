...
const LeaveDetailsPri = () => {
  ...
  const handleHODApproval = async () => {
    try {
      await axios.put(`http://localhost:8080/principal/approve/${leave_id}`);
  ...
  const handleHODRejection = async () => {
    try {
      console.log(`http://localhost:8080/principal/reject/${leave_id}`);
      await axios.put(`http://localhost:8080/principal/reject/${leave_id}`);
      // Refresh leave details or update state accordingly
      axios
        .get(`http://localhost:8080/inbox/${leave_id}`)
        .then((response) => {
      ...
      <div className="">
        ...
        {leave ? (
          <div className="">
            ...
            <p>Principal Approval Status: {leave.principal_approval_status}</p>

            {/* Buttons for HOD actions */}
            {leave.principal_approval_status === "PENDING" && (
              <>
...
export default LeaveDetailsPri;
